...
CACHE_VER = "v9bx"  # 2026-08-02 실험(exp_v9bx): v9b에서 "버그 2개만" 수정한 버전 — 신규 피처/필터 없음.
                    # ① 다이버전스 (가격,RSI) 짝 버그: 합성 극점 피봇 index를 실제 극점 봉으로 기록,
                    #    rsi2를 그 봉에서 읽음 (forming 봉이 극점이면 판정 스킵)
                    # ② 피봇 admission 룩어헤드: v10 브랜치(5a360bd)의 confirm_index 정순 인과 검출 이식
                    # 스키마/관측(25컬럼, 18+4차원)은 v9b와 동일 → v9b 베이스라인과 순수 비교용
...
def build_cache(symbol, recent_days=None, n=None, min_diff=None):
    ...
    # --- 전역 피봇 (버그② 수정: 시간 오름차순 인과 검출, v10 브랜치 이식) ---
    # 구버전은 내림차순 df에 검출기를 돌린 뒤 인덱스를 되돌렸다 — 스캔이 최신 봉에서
    # 과거로 진행되는 구조라 라이브 재현이 불가능했다. 정순 1회 순회로 검출하고
    ...
    for i in range(total_1m):
        ...
        # RSI 다이버전스 — 버그① 수정판: rsi2를 실제 극점 봉에서 읽고, 극점이 forming
        # 봉(index == idx_5m)이면 RSI 미확정이라 판정하지 않음. 그 외 조건(필터 없음)은
        # v9b와 동일 — "버그만 해소" 비교 목적이므로 min_gap/min_dist 등 신규 필터 미도입.
        rsi_prev = 50.0
        div_price_gap = 0.0
