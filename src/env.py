...
- exit_mode="rl" (기본, 풀 컨트롤): 결정 주기 매 1m 행, 행동 Discrete(4)
  {0: Hold, 1: Open-Long, 2: Open-Short, 3: Close}. 보상은 매 스텝 mark-to-market
  손익 변화량(dense reward). 2026-07-16 seed0 학습에서 "아무것도 안 함" 붕괴
  (검증 콜백 4회 연속 거래 0건, ent_coef 상향/에피소드 길이 조정 모두 무효) 확인됨.
...
class TradingEnvV9(gym.Env):
    ...
    def _step_rl(self, action):
        ...
        if action == ACT_LONG:
            if self.pos_dir < 0:
                self._close_position(self.closes[i], ts, "flip")
            if self.pos_dir == 0:
                self._open_position(+1, i)
