...
### <span style="color: #2E8B57;">피처 캐시 (`features_v9d2_{SYMBOL}.npy`, 행당 25컬럼 — 2026-08-04 v9d2)</span>
...
- 피처 캐시: `caches/features_v9d2_{SYMBOL}[_recent{N}d].npy` + `dist_report_v9d2_{SYMBOL}[...].json` (2026-08-04 v9d2 — RSI 다이버전스 3종을 `5m_rsi` 1분 컬럼 기반으로 재계산. 구 `features_v9d1_*`/`v9c_*`/`v9bx_*`/`v9b_*`/`v9_*`는 구 체크포인트 재평가용으로 보존. `--cache-ver`(train.py/eval.py 둘 다 지원)로 구 버전 명시 지정 가능)
