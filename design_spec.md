...
- 주요 컬럼: 파동 방향/시작·끝 가격/스케일/지속시간, RSI 다이버전스(발생 플래그 + rsi_previous + 가격 갭 + RSI 갭), relative_volume_strength(트레일링 500캔들 min-max 백분위 — V8의 0.0 하드코딩 버그 수정판), volatility_ratio, adx_5m/atr_5m(RMA 방식), fib_pos(파동 내 되돌림 위치), pre_hit_0382(0.382 선터치 플래그), wave_age_min.
...
| 9 | adx_5m | /100 |
| 10 | atr/close % | clip 0~3% → /3 |
| 11 | is_bullish | ±1 |
| 12 | fib_pos (되돌림 위치) | clip −1~2 → /2 |
| 13 | pre_hit_0382 | 0/1 |
| 14–15 | fib_pos 델타 (5분 전/15분 전 대비, 파동 교체 시 0) | clip ±1 |
| 16 | 현재 RSI (방향조정, 직전 마감 5m 캔들) — **2026-07-20 추가** | /100 |
| 17–19 | 트레일링 수익률 5m/15m/1h (ATR% 정규화 "몇 ATR 움직임") — **2026-07-20 추가** | clip ±3/±6/±12 ATR |
