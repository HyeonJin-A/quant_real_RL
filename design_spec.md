...
| ent_coef | 0.01 → 0.005 (CLI로 override 가능) | 기본값은 70% 지점까지 0.01 고정 유지 후 선형 감쇠. 2026-07-20 실측: 기본값으론 std가 클램프 상한(≈2.2~2.3)에 계속 붙어 승률이 27~29%를 못 넘음 — `--ent-coef-start 0.005 --ent-coef-hold-frac 0.2`로 낮추자 std가 실제로 0.7→1.35까지 내려가며 승률 37~40%(BTC/ETH 공통)로 안착, best 갱신 재개 (6장 이력 참고) |
...
가드: 월평균 거래수 < 3이면 best 후보 자격 박탈 (무거래=중립 함정 차단)
```
...
| 07-20 | **모델 선택을 월별 복리 log-multiple 평균−std로 교체** | 구 기준은 승률 항 지배로 "본전+균등"(9.5M, +28)이 "수익+편중"(50M, +418)을 이겼고 복리 MDD 99%도 통과시킴. 무거래 함정 가드(월 3거래) 포함. seed1 런부터 적용 |
...
- 런 이름: `v9_ppo_seed{S}_{MMDD-HHMM}` — **시작 시각(KST, 2026-07-20부터) 포함 유니크 (2026-07-19 도입)**. 이전의 `v9_ppo_seed{S}` 고정 이름은 재시작 시 TB 런 디렉토리·체크포인트가 이전 런 산출물을 덮어쓰는 사고가 있어 폐기.
- 모델: `models/{run_name}_{steps}_steps.zip` (주기 체크포인트), `{run_name}_best.zip` + `..._best_info.json` (검증 best), `{run_name}_final.zip` (학습 종료 시점)
- 로그: `logs/{run_name}_*/` (TensorBoard — `valid/{SYMBOL}/...` 지표군, `valid/BTC-USDT-SWAP/sel_monthly_log`가 모델 선택 점수 — 2026-07-20부터, 구 태그명 `v9_score_sel` 폐기). 폐기 런은 `logs_discarded/`로 이동 (TB 스캔 제외), 보존 가치가 있으면 구분되는 이름으로 `logs/`에 유지.
