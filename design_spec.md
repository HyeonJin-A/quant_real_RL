...
- **2026-08-05부터 1분봉에서 직접 검출**(이전엔 5분봉) — `find_dynamic_pivots` 자체는 캔들 단위와 무관한 범용 함수라 무수정, `prep_features.py` 호출부만 1분봉 DataFrame을 넘기도록 교체. `n`은 원래 5분봉 기준으로 그리드서치했으므로 1분봉 검출용으로는 x5가 필요한데, **`PIVOT_PARAMS`에 이미 x5된 값으로 저장**해뒀다(최초엔 호출부에서 매번 `n*5`로 환산했다가 상수 정의 시점에 미리 곱해두는 쪽으로 리팩터링 — 호출부는 `n`을 그대로 씀, 결과값 동일해 캐시 재생성 불필요). `min_diff`는 가격 비율이라 스케일 불변, 그대로.
- **심볼별 피봇 파라미터** (2026-07-17 그리드서치로 확정 — n은 5분봉 기준 그리드서치 원값에 x5해서 `PIVOT_PARAMS`에 저장):
  | 심볼 | n (5분봉 그리드서치 원값) | `PIVOT_PARAMS`에 저장된 n(x5, 1분봉 기준) | min_diff |
  |---|---|---|---|
...
### <span style="color: #2E8B57;">피처 캐시 (`features_v9d4_{SYMBOL}.npy`, 행당 25컬럼 — 2026-08-05 v9d4)</span>
...
- **레버리지는 `--leverage` 고정 상수**(CLI 기본값 **3.0**, `train.py`에서 env.py 생성자 기본값 20.0을 덮어씀. 2026-07-28에 5.0으로 한 차례 올렸었으나 실제 채택된 최신 베이스라인(0728-1924, 5장 참고)은 leverage=3/explore_bonus=0.003 조합이라 코드 기본값도 3.0으로 유지·정정됨 — 5.0/0.005는 실험만 되고 기본값으로 확정되지 못함. 2026-07-31 확인. 10배도 실험했으나 청산 위험이 과도해 실전 배제, 6장 참고) — 진입마다 정책이 고르지 않음. Discrete 출력(3개 숫자 중 하나 선택)엔 연속값을 담을 자리가 없어, 혼합형(이산+연속) 정책 없이는 구조적으로 불가능 (로드맵 항목으로 남김).
...
- 피처 캐시: `caches/features_v9d4_{SYMBOL}[_recent{N}d].npy` + `dist_report_v9d4_{SYMBOL}[...].json` (2026-08-05 v9d4 — 5m 데이터 완전 제거, 피봇 검출 자체가 1분봉 네이티브. 구 `features_v9d3_*`/`v9d2_*`/`v9d1_*`/`v9c_*`/`v9bx_*`/`v9b_*`/`v9_*`는 구 체크포인트 재평가용으로 보존. `--cache-ver`(train.py/eval.py 둘 다 지원)로 구 버전 명시 지정 가능)
