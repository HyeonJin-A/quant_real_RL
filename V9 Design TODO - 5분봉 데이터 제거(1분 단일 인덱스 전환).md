...
`CACHE_VER=v10`, `v9d4`에서 이름만 변경 — 아래 6장 참고). 이 문서의 목표(1분 단일 인덱스 아키텍처 전환) 달성 — `prep_features.py`가
`_1m.csv`만 읽음.**
...
`CACHE_VER`: v9d3 → **v9d4**(사용자 지정 — 피봇 검출 방식 자체가 바뀌는 질적 전환이지만 v9d 계열 번호로 유지) → 이후 **v10**으로 개명(2026-08-05, 사용자 지정 — "앞으로 캐시 버전은 run_name(`v10_maskablerl` 등)과 동일 네이밍을 따라간다." 값 변경 없이 `features_v9d4_*.npy`/`dist_report_v9d4_*.json`을 `v10`으로 파일명만 rename, `CACHE_VER` 상수만 교체).
`design_spec.md` 반영 완료.
